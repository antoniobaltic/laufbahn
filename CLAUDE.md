...
| `surface-stage` | Large orientation-first hero surface for major routes |
| `surface-rail` | Secondary sticky rail / side-panel surface |
| `surface-muted` | Softer inset section for helper areas/forms |
| `eyebrow-badge` | Small rounded metadata chip for quiet context cues |
| `segmented-control` | Focus-switcher shell for dense multi-module views |
| `segmented-pill` | Focus-switcher button inside a segmented control |
| `stage-visual` | Dark premium visual panel for mood, guidance, or future imagery |
| `metric-cloud` | Light summary card for metrics and focus shortcuts |
...
### Stage & Image Layers
- `--marketing-track-image` is the reserved hook for the future landing-page running-track artwork.
- `--page-stage-image` is the route-level image hook for auth, detail heroes, and future premium moments.
- Stage surfaces must still look complete when no image is supplied. Gradients, lane-line textures, and overlays are the fallback, not a broken state.
- Intended artwork direction:
  - landing hero: overhead running track in warm clay/orange tones with visible curved lane lines and generous negative space for the headline
  - auth left panel: a tighter crop from the same world, more atmospheric and less literal
  - detail/premium stages: darker abstract track lines, paper grain, or stadium-light glows instead of generic office/product screenshots
- Do not place busy imagery directly behind dense copy. Images should anchor mood and rhythm, not compete with hierarchy.
...
- Placeholder or obviously fake domains such as `example.com` should also fall back directly to a monogram instead of triggering failed favicon requests.
...
- The left panel should feel like the same visual world as the landing hero, not like a disconnected brand screen.
...
- The sidebar should feel like a freestanding object inside the page, not a stretched admin slab welded to the viewport.
...
- If only one next-step prompt is visible, let it span the available width instead of forcing a thin grid cell with dead space beside it.
...
- The first viewport should follow a three-part rhythm:
  - orientation hero left
  - visual/focus stage right
  - sticky decision rail for status and workflow
- The detail page should be focus-first, not “all modules at once”. Use segmented switching so one deeper area is dominant at a time.
...
- The dark stage visual in the hero must remain readable even without a custom image asset. Contrast is mandatory, not optional polish.
...
- The overview should begin with a library-style hero plus one simple view switch such as `Bibliothek` / `Varianten`, not with tree mechanics or advanced controls.
